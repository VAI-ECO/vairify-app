...
import { ArrowLeft, Eye, EyeOff, Info, Check, AlertCircle, ChevronDown, ChevronUp } from "lucide-react";
...
const Registration = () => {
  ...
  useEffect(() => {
    if (otpMethod === 'phone' && touched.phone) {
  ...
  }, [otpMethod, formData.phone, touched.phone]);
  ...
  const validateField = (name: keyof FormData, value: any): string | undefined => {
    switch (name) {
      ...
      case 'phone':
        if (otpMethod === 'phone') {
          if (!value) return 'Phone number is required for SMS verification';
      ...
      <div className="flex-1 flex items-center justify-center p-6 overflow-y-auto">
        <div className="w-full max-w-md animate-fade-in">
          ...
          <form onSubmit={handleSubmit} className="space-y-6 bg-white/10 backdrop-blur-sm rounded-3xl p-8 border border-white/20">
            {/* Existing VAI Number - At the top with radio button */}
            <div className="space-y-3">
              <RadioGroup 
                value={formData.hasExistingVAI ? "yes" : "no"}
                onValueChange={(value) => {
                  const hasVAI = value === "yes";
                  setFormData(prev => ({ 
                    ...prev, 
                    hasExistingVAI: hasVAI,
                    existingVAINumber: hasVAI ? prev.existingVAINumber : ''
                  }));
              ...
              >
                <div className="flex items-center space-x-3">
                  <RadioGroupItem 
                    value="yes" 
                    id="hasVAI-yes" 
                    className="border-white/50 data-[state=checked]:bg-white data-[state=checked]:border-white h-5 w-5 rounded-full [&>span>svg]:fill-primary [&>span>svg]:text-primary"
                  />
                  <Label htmlFor="hasVAI-yes" className="text-white text-sm font-medium cursor-pointer">
                    I have another a V.A.I. number from another Chainpass V.A.I. platform
                  </Label>
                </div>
              </RadioGroup>

              <Collapsible 
                open={formData.hasExistingVAI} 
                onOpenChange={(open) => {
                  setFormData(prev => ({ 
                    ...prev, 
                    hasExistingVAI: open,
                    existingVAINumber: open ? prev.existingVAINumber : ''
                  }));
                  setTouched(prev => ({ ...prev, hasExistingVAI: true }));
                }}
              >
                <CollapsibleContent>
                  <div className="space-y-2 mt-3 p-4 bg-white rounded-xl border border-gray-200 shadow-sm">
                    <Label htmlFor="existingVAINumber" className="text-gray-900 text-sm font-medium">
                      Your VAI Number <span className="text-gray-600">(from ChainPass or another platform)</span>
                    </Label>
                    <Input
                      ...
                      className="h-12 bg-white border-gray-300 text-gray-900 placeholder:text-gray-400 focus:bg-white focus:border-primary font-mono"
