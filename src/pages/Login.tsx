...
type LoginPreference = "facial" | "password" | "phone_otp";
...
const Login = () => {
  ...
  const updateLoginPreference = async (userId: string, preference: LoginPreference) => {
    const { error } = await supabase
      .from('profiles')
      .update({ login_preference: preference })
      .eq('id', userId);
  ...
  const completeLoginFlow = async (userId: string, preference: LoginPreference) => {
    await updateLoginPreference(userId, preference);
  ...
  const handleSendOTP = async () => {
    ...
    if (!normalizedPhone) {
      toast({
        ...
        description: "Include your country code (e.g., +14155552671).",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: "OTP Sent",
        description: `Code sent to ${normalizedPhone}`,
      });
      setOtpCode("");
    } catch (error: any) {
      toast({
  ...
  const handleVerifyOTP = async () => {
    ...
    if (!normalizedPhone) {
      toast({
        title: "Invalid phone number",
        description: "Enter the same number used to request the code.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (otpCode.length < 4) {
      toast({
        ...
        description: "The SMS code should be 4-6 digits.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      await completeLoginFlow(data.user.id, "phone_otp");
      toast({
        title: "Welcome back!",
  ...
  const handleFacialRecognitionSuccess = async (capturedImage: string) => {
    // Facial recognition successful - authenticate user via edge function
    setLoading(true);
    try {
      const chainpassUrl = import.meta.env.VITE_CHAINPASS_URL;
      const chainpassKey = import.meta.env.VITE_CHAINPASS_SERVICE_KEY;

      if (!chainpassUrl || !chainpassKey) {
        throw new Error("ChainPass integration is not configured.");
      }

      const chainpassResponse = await fetch(
        `${chainpassUrl}/functions/v1/verify-vai-facial`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'x-service-key': chainpassKey,
          },
          body: JSON.stringify({
      ...
      if (!chainpassResponse.ok) {
        throw new Error("Unable to reach ChainPass for verification.");
      }

      const { match, confidence } = await chainpassResponse.json();

      if (!match) {
        throw new Error(
          confidence
            ? `Face mismatch detected (confidence ${confidence}%).`
  ...
  const handlePasswordLogin = async () => {
    ...
    try {
      ...
      await completeLoginFlow(
        data.user.id,
        identityMethod === 'vai' ? 'facial' : 'password'
      );
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "Continue to set up your profile.",
      });
                    ...
                    <>
                      {showFacialRecognition && storedPhotoUrl ? (
                        <FacialRecognitionLogin
                          ...
                          onSuccess={(image) => handleFacialRecognitionSuccess(image)}
